...
export default function Timeline({
  ...
  onSelect: (m: ResolvedMilestone, lane: ResolvedLane) => void;
}) {
  const layout = useMemo(
    () => computeLayout(board, hiddenLaneIds, weekWidth),
    [board, hiddenLaneIds, weekWidth]
  );

  const { trackWidth, lanesHeight, visibleLanes, byId } = layout;

  // Sequence arrows: consecutive dated milestones within each lane.
  const seqArrows = useMemo<SeqArrow[]>(() => {
    const arrows: SeqArrow[] = [];
    for (const lane of visibleLanes) {
      const placed = lane.milestones
        .map((m) => byId.get(m.id))
        .filter((p): p is MilestonePlacement => Boolean(p))
        .sort((a, b) => a.cx - b.cx);
      for (let i = 0; i < placed.length - 1; i++) {
        ...
        const x2 = b.left;
        if (x2 - x1 > 3) arrows.push({ x1, x2, y: a.cy });
      }
    }
    return arrows;
  }, [visibleLanes, byId]);

  // Cross-lane dependency arrows.
  const depArrows = useMemo<DepArrow[]>(() => {
    ...
    for (const dep of board.dependencies) {
      ...
      if (!from || !to || from.laneIndex === to.laneIndex) continue;
      ...
      const path = `M ${from.cx} ${startY} C ${from.cx} ${midY}, ${to.cx} ${midY}, ${to.cx} ${endY}`;
      arrows.push({ path, fromTitle: from.milestone.title, toTitle: to.milestone.title });
    }
    return arrows;
      ...
      <div className="relative" style={{ width: LABEL_WIDTH + trackWidth }}>
        ...
        <div className="sticky top-0 z-30 flex" style={{ height: RULER_HEIGHT }}>
          ...
          <div className="relative bg-slate-50" style={{ width: trackWidth, height: RULER_HEIGHT }}>
            ...
            {showToday && (
              <div
                className="absolute top-1 z-20 -translate-x-1/2 rounded bg-rose-500 px-1.5 py-0.5 text-[9px] font-bold text-white shadow"
                style={{ left: todayX }}
              >
                TODAY
              </div>
        ...
        <div className="flex">
            ...
            {visibleLanes.map((lane) => {
              ...
              return (
                ...
                >
                  ...
                  {lane.launchLabel && (
                    <div className="mt-1 text-[11px] font-semibold opacity-90">{lane.launchLabel}</div>
                  )}
          ...
          <div className="relative" style={{ width: trackWidth, height: lanesHeight }}>
            ...
            {visibleLanes.map((lane, i) => (
              <div
                ...
                style={{
                  ...
                  height: LANE_HEIGHT,
                  width: trackWidth,
                  backgroundColor: tint(lane.color, 0.035),
                }}
              />
            ))}

            {/* Gridlines + today (background SVG) */}
            <svg className="pointer-events-none absolute inset-0" width={trackWidth} height={lanesHeight}>
              {board.weeks.map((w) => (
                <line
                  key={w.index}
                  x1={w.index * weekWidth}
                  y1={0}
                  x2={w.index * weekWidth}
                  y2={lanesHeight}
                  stroke="#cbd5e1"
                  strokeWidth={1}
                  strokeDasharray="2 4"
              ...
              {showToday && (
                <line x1={todayX} y1={0} x2={todayX} y2={lanesHeight} stroke="#f43f5e" strokeWidth={2} />
              )}
            </svg>
            ...
            <svg className="pointer-events-none absolute inset-0 z-10" width={trackWidth} height={lanesHeight}>
              <defs>
                <marker id="arrow-seq" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse">
            ...
            <div className="absolute inset-0 z-20">
              {layout.placements.map((p) => {
                const lane = visibleLanes[p.laneIndex];
                return (
                  <MilestoneCard
                    key={p.milestone.id}
                    placement={p}
                    color={lane.color}
                    onClick={() => onSelect(p.milestone, lane)}
                  />
                );
              })}
            </div>
